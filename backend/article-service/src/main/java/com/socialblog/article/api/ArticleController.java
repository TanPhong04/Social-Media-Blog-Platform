...
public class ArticleController {
    ...
    Response bySlug(@PathVariable String slug) {
    ...
    @PutMapping("/{id}")
    Response update(@PathVariable UUID id, @AuthenticationPrincipal Jwt jwt, @Valid @RequestBody WriteRequest r) {
        return service.update(id, user(jwt), r);
    }

    @PostMapping("/{id}/publish")
    Response publish(@PathVariable UUID id, @AuthenticationPrincipal Jwt jwt) {
        return service.publish(id, user(jwt));
    }

    @PostMapping("/{id}/unpublish")
    Response unpublish(@PathVariable UUID id, @AuthenticationPrincipal Jwt jwt) {
        return service.unpublish(id, user(jwt));
    }
