...
public class GlobalExceptionHandler {
    ...
    ResponseEntity<ErrorResponse> api(ApiException e, HttpServletRequest r) {
        return ResponseEntity.status(e.status()).body(new ErrorResponse(Instant.now(), e.status().value(), e.code(), e.getMessage(), r.getRequestURI(), Map.of()));
    }
    ...
    ResponseEntity<ErrorResponse> validation(MethodArgumentNotValidException e, HttpServletRequest r) {
        ...
        return ResponseEntity.badRequest().body(new ErrorResponse(Instant.now(), 400, "VALIDATION_FAILED", "Request validation failed", r.getRequestURI(), f));
    }
}
